    ...
    ($($t:ty)*) => ($(
        impl Matrix<$t> {
            ...
            pub fn lu_decomposition(&self) -> (Matrix<$t>,Matrix<$t>,Matrix<$t>,usize){
                ...
                let mut matrix_l = Matrix::<$t>::fill(self.shape.0, self.shape.0, 0.0);
                let mut matrix_u = Matrix::<$t>::fill(self.shape.0, self.shape.0, 0.0);
                let mut matrix_p = Matrix::<$t>::fill_diagonal(self.shape.0, 1.0);
    ...
    ($($t:ty)*) => ($(
        impl Matrix<$t> {
            fn lu_permut(matrix_a : &mut Matrix<Ratio<$t>>, col: i32, num_piv : &mut usize) -> i32{
                let mut ind_max=col;
                for i in (col+1)..matrix_a.shape.0{
                    if matrix_a.get(ind_max,col)< matrix_a.get(i as i32,col) && matrix_a.get(ind_max,col) > &(Ratio::new(-1, 1) * matrix_a.get(i as i32,col)) {ind_max = i as i32;}
                }
                if (ind_max!=col) {*num_piv+=1}
                matrix_a.swap(col, ind_max, true);
                ind_max
