...
impl<T : Float + FromPrimitive> LuDecomposition<T> for Matrix<T> {
    fn lu_permut(matrix_a : &mut Matrix<T>, col: i32, num_piv : &mut usize) -> i32{
        let mut ind_max=col;
...
impl<T : Signed + std::cmp::PartialOrd + Copy> Matrix<T> {
    fn lu_permut(matrix_a : &mut Matrix<T>, col: i32, num_piv : &mut usize) -> i32{
        let mut ind_max=col;
        for i in (col+1)..matrix_a.shape.0{
            if matrix_a.get(ind_max,col).abs()< matrix_a.get(i as i32,col).abs() {ind_max = i as i32;}
...
impl<T : Float + FromPrimitive + Copy + std::cmp::PartialOrd> Matrix<T> {
