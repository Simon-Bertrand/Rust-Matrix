...
impl<T : std::fmt::Display + std::cmp::PartialOrd + std::fmt::Debug> std::fmt::Display for Matrix<T> {
    fn fmt(&self, _f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let spaces = count_digits(&self);
            ...
            for val in self.values.iter() {      
                if i==0 {
                    print!("{}", "|    ");
                    print!("{val:>width$}", val=format!("{:.3}",  val), width=4);
                }
                    ...
                    print!("{val:>width$}", val=format!("{:.3}",  val), width=spaces);
                }
...
impl<T : std::fmt::Display + std::cmp::PartialOrd +  std::fmt::Debug> Matrix<T> {
    pub fn show(&self) {
        println!("{}", self)
    }
}
